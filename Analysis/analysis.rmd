...
modelsummary::modelsummary(
  ...
  shape = formula(term + statistic ~ model), 
...
```{r}
ggplot(data_aggregated, aes(CompletedThrows, RadialError)) +
  geom_jitter(width = 0.15, color = "#2979FF", alpha = 0.6) +
  scale_x_continuous(breaks = c(1, c(5, 10, 15, 20)), minor_breaks = seq(1, 20)) +
  annotate("rect", xmin = 0, xmax = 10.5, ymin = 0, ymax = Inf, alpha = 0.2, fill = "black") +
  ...
  geom_line(aes(x = ifelse(Condition == "Dark room", 5, 15), y = colMeans(posterior_predict(fit, data_aggregated)))) +
  labs(x = "Trial", y = "Radial Error (cm)") +
  theme(
